...
export default function About() {
    ...
    return (
    <div className="flex min-h-screen text-white gap-x-0">

        {/* Left Side: Breif Description */}
        <div className="flex flex-col justify-center items-center w-full md:w-1/2 px-8 py-12">
            <div className="bg-indigo-400/10 backdrop-blur-sm rounded-2xl px-10 py-15 shadow w-full max-w-2x1">
            <h1 className="text-4xl font-bold text-indigo-300 mb-4 text-center">Welcome to Day Planner</h1>
        ...
        <div className="flex flex-col justify-center items-center w-full md:w-1/2 px-8 py-12 gap-6">
            ...
            <div className="bg-gray-800 rounded-2xl shadow p-6 w-full max-w-lg">
            <button
                onClick={() => setShowFeatures((prev) => !prev)}
                className="flex items-center text-2xl font-bold text-indigo-300 focus:outline-none"
            >
                <span className="text-indigo-400 text-3xl mr-2">{showFeatures ? "−" : "+"}</span>
                Project Features
            </button>
            {showFeatures && (
            ...
            <button
                ...
                className="flex items-center text-2xl font-bold text-indigo-300 focus:outline-none"
            >
                <span className="text-indigo-400 text-3xl mr-2">{showDev ? "−" : "+"}</span>
                Meet the Developers
            </button>
            {showDev && (
            ...
            <button
                ...
                className="flex items-center text-2xl font-bold text-indigo-300 focus:outline-none"
            >
                <span className="text-indigo-400 text-3xl mr-2">{showMotivation ? "−" : "+"}</span>
                Project Motivation
            </button>
            {showMotivation && (
                <div className="mt-4 ml-8 text-white-100">
